...
FILE_PREDICT = "titanic_predictions2.csv"

def input_fn(file_path, repeat_count=1, shuffle=False):
  # define mapping function
  def decode_csv(line):
    parsed_line = tf.decode_csv(line, [[0], [0], [0], [""], ["unknown"], [0.], [0], [0], [""], [0.], ["U"], ["U"]])
    label = parsed_line[1]
...
def predict_input_fn():
  ...
  prediction_set = pd.read_csv("titanic_predictions2.csv")
...
def main():
  ...
  Embarked = tf.feature_column.categorical_column_with_vocabulary_list(
      "Embarked", ["C", "Q", "S", "U"])
  cabin_prefix = tf.feature_column.categorical_column_with_vocabulary_list(
  ...
  feature_cols = [Pclass,
                  ...
                  tf.feature_column.indicator_column(cabin_prefix),
                  age_buckets]

  # set up classifier
  m = tf.estimator.DNNClassifier(
        model_dir="/tmp/titanicDNN2",
